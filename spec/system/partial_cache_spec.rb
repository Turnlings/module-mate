...
RSpec.describe 'Partial caching', type: :system do
  ...
  context 'when you update an exam result' do
    it 'bubbles up to updating the dashboard achieved stat' do
      ...
      visit root_path
      expect(page).to have_css('.score-section .main .stat', text: '0.0%')
      ...
      expect(page).to have_css('.score-section .main .stat', text: expected_score)
    end
    ...
    it 'bubbles up to updating the years partial cache' do
      ...
      visit root_path
      within('#years') do
        expect(page).to have_css('.actual', text: '0%')
      end
      ...
      within('#years') do
        expect(page).to have_css('.actual', text: expected_year_score)
      end
    end
  end
end
